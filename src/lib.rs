...
pub fn keccak_p_inplace<const ROUNDS: usize>(state: &mut [u64; 25]) {
    let ptr = state as *mut [u64; 25];
    unsafe { keccak_p::<ROUNDS>(&mut *ptr, &*ptr) }
}

pub fn keccak_f(dst: &mut [u64; 25], src: &[u64; 25]) {
    keccak_p::<24>(dst, src)
}

pub fn keccak_f_inplace(state: &mut [u64; 25]) {
    keccak_p_inplace::<24>(state)
}
...
#[cfg(test)]
mod tests {
    use crate::keccak_f_inplace;

    #[test]
    fn keccak_f_test() {
        let mut input = [0; 25];
        keccak_f_inplace(&mut input);
        #[rustfmt::skip]
        assert_eq!(input, [
